...
# It seems `_get_compiled_pattern` caching limits the benefit if text length is small
# Let's adjust benchmark to accurately reflect dictionary overhead
# The major cost is `for k, v in self._replacements.items(): combined_rules[k] = v`
# AND the apply_replacements `isinstance(rules, dict)` which creates overhead
# iterating the dict items vs iterating the list items

class OldTranscriber:
    def __init__(self):
        self._replacements = {f"default{i}": f"rep{i}" for i in range(50)}
        self._dict_data = DictionaryData()
        self._dict_data.replacements = {f"user{i}": f"urep{i}" for i in range(50)}
        self._dict_data.compiled_replacements = [
            (re.compile(rf"(?<!\w)user{i}(?!\w)", re.IGNORECASE), f"urep{i}") for i in range(50)
        ]
...
if __name__ == "__main__":
    o = OldTranscriber()
    n = NewTranscriber()
    text = "This is default25, but user45 and hip a need foo and nevermind."

    print("Benchmarking Heavy Dict Overhead...")
