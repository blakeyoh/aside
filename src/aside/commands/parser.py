...
def _get_last_char(parts: list[str]) -> str:
    """Return the final character of the last non-empty string in parts, or ''."""
...
def _append_dictation_output(parts: list[str], cmd: Command) -> None:
    ...
    # Check the last character without full string concatenation (O(1) vs O(N))
    last_char = _get_last_char(parts)
    if last_char in _ARTIFACT_PUNCT:
        # The original logic specifically looked at parts[-1] after rstrip,
        # which means it only stripped artifact punctuation if it was in the VERY LAST chunk.
        # But wait, rstrip removed empty chunks from the end if the last was empty before rstrip.
        # So we just do exactly what original did: look at parts[-1]
        if parts:
