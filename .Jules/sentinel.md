...
## 2026-05-27
- **Vulnerability:** Unsanitized User Input Written to Dictionary File
- **Learning:** User input from UI forms or IPC messages was written directly into a configuration file (`dictionary.txt`) without removing newline characters (`\n`, `\r`). This allowed a malicious or errant user input to inject unauthorized configuration rules or corrupt the file format entirely.
- **Prevention:** When constructing file entries from user input that rely on line breaks as delimiters, always strip or replace newline and carriage return characters before appending to the file.
