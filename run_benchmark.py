...
class OldTranscriber:
    def __init__(self):
        self._replacements = {"alright": "all right", "nevermind": "never mind"}
        self._dict_data = DictionaryData()
        self._dict_data.replacements = {f"user{i}": f"urep{i}" for i in range(20)}
        self._dict_data.compiled_replacements = [
            (re.compile(rf"(?<!\w)user{i}(?!\w)", re.IGNORECASE), f"urep{i}") for i in range(20)
        ]
...
if __name__ == "__main__":
    o = OldTranscriber()
    n = NewTranscriber()
    text = "This is alright, but user15 and nevermind are here." * 10

    print("Benchmarking Old vs New Transcriber...")
